...
- match_type の選び方（重要: 正確に選ぶこと）:
  - "from": 「〇〇から」「〇〇さんの」など送信者が特定されている場合は必ずこれを使う
  - "subject": 件名にキーワードが含まれる場合のみ
  - "keyword": 送信者も件名も不明で本文含め広く探したい場合のみ（誤マッチが多くなるので慎重に）
- 「〇〇からの△△」のように送信者+内容が指定された場合は、match_type="from"でpatternに送信者名を設定する（内容の条件はdescriptionに記載するだけでよい）
